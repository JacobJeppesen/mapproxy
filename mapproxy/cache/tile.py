    ...
    def _create_single_tile(self, tile, dimensions=None):
        ...
        with self.tile_mgr.lock(tile):
            if not self.is_cached(tile, dimensions=dimensions):
                ...
                # Sometimes, when we try to upscale a tile, we get a white tile. Don't know why it happens, and it only
                # happens when upscaling. If we request the "original" resolutions from the source, we never encounter
                # this error.
                else:
                    # Check that the source object has a source attribute (true if it contains an image)
                    if hasattr(source, 'source'):
                        # Check if we got a PIL image
                        from PIL import Image
                        if isinstance(source.source, Image.Image):
                            # Do band statistics to figure out if we got a white tile
                            from PIL import ImageStat
                            stat = ImageStat.Stat(source.source)
                            # print(f"Mean band values: {stat.mean}")
                            # If the first 3 bands is 255.0, the tile is white
                            if stat.mean[0] == 255.0 and stat.mean[1] == 255.0 and stat.mean[2] == 255.0:
                                # print("White tile encountered")
